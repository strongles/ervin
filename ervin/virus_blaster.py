...
def get_newest_ftp_file_timestamp(ftp_handle):
    file_stats = [
        ftp_handle.stat(file) for file in ftp_handle.listdir(ftp_handle.curdir)
    ]
...
def get_virus_db_files():
    ...
    with ftputil.FTPHost(VIRUS_DB_SERVER, "anonymous", "") as virus_serv:
        ...
        with progressbar.ProgressBar(
            ...
            type="percentage",
            prefix="Downloading virus DB source files: ",
        ) as bar:
...
def get_top_virus_hit(record, db=VIRUS_DB_DEFAULT):
    config = get_config()
    ...
    try:
        return parsed_result["report"]["results"]["search"]["hits"][0]["description"][
            0
        ]["title"]
    except IndexError:
        return "not_found"
...
def run_virus_blaster(filename=None, db=None, output_dir=None, run_ts=None):
    ...
    with progressbar.ProgressBar(
        max_value=len(input_data),
        type="percentage",
        prefix="Blasting against Viruses: ",
    ) as bar:
        for count, file_record in enumerate(input_data):
            bar.update(count)
            top_hit = get_top_virus_hit(file_record, db)
            matched_virus_files.add(
                print_virus_results_to_file(
                    top_hit, [file_record], run_stamp, output_dir
                )
            )
