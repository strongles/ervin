...
def _length_requirement(probe_list, args):
    return [
        ...
        if probe.alignment_length > args.alignment_len_threshold
    ]
...
def run_probe_blaster(
    file,
    genome_db,
    align_threshold,
    e_value,
    run_ts=None,
    output_dir=TEMP_PROBE_BLASTER,
):
    run_time = run_ts if run_ts else format_timestamp_for_filename()
    align_len = (
        align_threshold if align_threshold else DEFAULT_ALIGNMENT_LENGTH_THRESHOLD
    )
    ...
    with progressbar.ProgressBar(
        max_value=len(probe_records),
        type="percentage",
        prefix="Blasting against Genome DB: ",
    ) as bar:
        for count, probe_record in enumerate(probe_records):
            ...
            output_file = print_results(
                filtered_results, probe_record["title"], run_time, output_dir
            )
