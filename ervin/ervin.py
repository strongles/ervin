...
def run():
    ...
    probe_finder_fasta, fasta_count = run_probe_finder(
        blasted_probes, args.alignment_len_threshold, run_ts
    )
    _, virus_to_counts = run_virus_blaster(
        probe_finder_fasta, args.virus_database, args.output_dir, run_ts
    )
    write_summary_file(
        probe_count, fasta_count, virus_to_counts, args.output_dir, run_ts
    )
