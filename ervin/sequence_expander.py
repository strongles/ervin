...
def validate_settings(conf_settings, parsed_args):
    validation = {
        arg: (arg not in conf_settings and arg not in parsed_args)
        for arg in REQUIRED_ARGS
    }
    print(validation)
    if any(
        [(arg not in conf_settings and arg not in parsed_args) for arg in REQUIRED_ARGS]
    ):
        raise IncompleteArgsException(
            f"Args missing. Ensure all are provided: "
...
def determine_ltr_hits(blast_results, db_path):
    ...
    for potential_ltr in potential_ltrs:
        ...
        print(
            f"Full sequence start and end: {full_segment_start}-{full_segment_end} "
            f"Sequence length should be {full_segment_end-full_segment_start}"
        )
        if full_segment_end - full_segment_start <= ENV_UPPER:
            scaf_record = ScafRecord(
                accession_id=parsed_title["acc_id"],
...
def run():
    ...
    for record in records:
        ...
        scaf_record = ScafRecord(
            ...
            segment=get_from_db(
                new_range_start, new_range_end, record["accession_id"], db_filepath
            ),
        )
